...
public final class Parser {
    ...
    private ActorLang parseActor() {
        ...
        if (current.isIdent(IMPLEMENTS_VALUE)) {
            ...
            protocolType = parseIntersectionProtocolType();
        }
        ...
        LexerToken endToken = acceptEndToken();
        if (name != null) {
            return new ActorStmt(name, params, protocolType, body, actorToken.adjoin(endToken));
        } else {
            return new ActorExpr(params, protocolType, body, actorToken.adjoin(endToken));
        }
    }
    ...
    private FieldType parseFieldType(IntegerCounter nextImpliedFeature) {
        ...
        if (featureOrValueType instanceof MethodType methodType) {
            ...
            featureAsType = new StrAsType(name.asStr(), name);
    ...
    private ProtocolType parseIntersectionProtocolType() {
        ProtocolType answer = parseProtocolTypeExpr();
        if (answer == null) {
            return null;
        }
        while (currentToken.isOneCharSymbol(AND_OPER_CHAR)) {
            nextToken(); // accept '&' token
            ProtocolType right = parseProtocolTypeExpr();
            if (right == null) {
                throw new ParserError(PROTOCOL_EXPECTED, currentToken);
            }
            answer = new IntersectionProtocolType(answer, right, answer.adjoin(right));
        }
        return answer;
    }
    ...
    private NewExpr parseNew() {
        ...
        TypeCtor typeCtor = parseTypeCtor();
        current = currentToken;
        if (!current.isOneCharSymbol(L_PAREN_CHAR)) {
            throw new ParserError(L_PAREN_EXPECTED, current);
        }
        ...
        nextToken(); // accept ')' token
        return new NewExpr(typeCtor, args, newToken.adjoin(endNewToken));
    }
    ...
    private ProtocolStmt parseProtocol() {
        ...
        nextToken(); // accept '=' token
        ProtocolType body = parseIntersectionProtocolType();
        return new ProtocolStmt(name, typeParams, body, typeToken.adjoin(body));
    }

    private ProtocolTypeCtor parseProtocolTypeCtor(LexerToken identToken) {
        Ident ident = tokenToIdent(identToken);
        IdentAsProtocolType name = IdentAsProtocolType.create(ident, currentToken);
        LexerToken current = currentToken;
        List<Type> typeArgs;
        ...
        return new ProtocolTypeCtor(name, typeArgs, identToken.adjoin(endProtocolApply));
    }

    private ProtocolType parseProtocolTypeExpr() {
        LexerToken current = currentToken;
        if (current.isIdent()) {
            LexerToken identToken = current;
            Ident name = tokenToIdent(current);
            IdentAsProtocolType identAsProtocolType = IdentAsProtocolType.create(name, identToken);
            current = nextToken(); // accept IDENT token
            if (current.isOneCharSymbol(L_BRACKET_CHAR)) {
                return parseProtocolTypeCtor(identToken);
            } else {
                return identAsProtocolType;
            }
        } else if (current.isOneCharSymbol(L_BRACE_CHAR)) {
            return parseProtocolTypeStruct();
        }
        return null;
    }

    private ProtocolTypeHandler parseProtocolTypeHandler() {
        LexerToken current = currentToken;
        if (current.isIdent(ASK_VALUE)) {
            ...
            return new ProtocolTypeAskHandler(pat, responseType, askSpan);
        } else if (current.isIdent(TELL_VALUE)) {
            nextToken(); // accept 'tell' token
            Pat pat = parsePat();
            return new ProtocolTypeTellHandler(pat, current.adjoin(pat));
        } else if (current.isIdent(STREAM_VALUE)) {
            ...
            Type responseType = parseResultTypeRequired();
            SourceSpan askSpan = responseType == null ? current.adjoin(pat) : current.adjoin(responseType);
            return new ProtocolTypeStreamHandler(pat, responseType, askSpan);
        } else {
            return null;
        }
    }

    private ProtocolTypeStruct parseProtocolTypeStruct() {
        LexerToken structToken = currentToken;
        nextToken(); // accept '{' token
        List<ProtocolTypeHandler> handlers = new ArrayList<>();
        ProtocolTypeHandler handler = parseProtocolTypeHandler();
        LexerToken current = currentToken;
        if (handler != null) {
            handlers.add(handler);
            while (current.isOneCharSymbol(COMMA_CHAR)) {
                nextToken(); // accept ',' token
                handler = parseProtocolTypeHandler();
                current = currentToken;
                if (handler != null) {
                    handlers.add(handler);
                } else {
        ...
        return new ProtocolTypeStruct(handlers, structSpan);
    }
    ...
    /*
     * Type construction occurs in two contexts:
     ...
     */
    private TypeCtor parseTypeCtor() {
        LexerToken identToken = currentToken;
        nextToken(); // accept IDENT token
        return parseTypeCtor(identToken);
    }

    private TypeCtor parseTypeCtor(LexerToken identToken) {
        Ident ident = tokenToIdent(identToken);
        ...
        return new TypeCtor(name, typeArgs, identToken.adjoin(endTypeApplyToken));
    }
