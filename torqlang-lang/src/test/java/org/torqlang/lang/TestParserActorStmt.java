...
public class TestParserActorStmt {
    ...
    public void testWithSelfProtocol() {
        ...
        Parser p = new Parser("actor MyActor() implements GetInt in handle ask 'get' -> Int32 in a end end");
        StmtOrExpr sox = p.parse();
        ActorStmt actorStmt = (ActorStmt) sox;
        assertSourceSpan(actorStmt, 0, 75);
        assertEquals(Ident.create("MyActor"), actorStmt.name.ident);
        // Test format
        String expectedFormat = """
            actor MyActor() implements GetInt in
                handle ask 'get' -> Int32 in
                    a
                end
            end""";
        ...
        assertInstanceOf(IdentAsProtocolType.class, actorStmt.protocolType);
        IdentAsProtocolType protocol = (IdentAsProtocolType) actorStmt.protocolType;
        assertEquals("GetInt", protocol.ident().name);
        // Test handlers
        assertEquals(1, actorStmt.askHandlers().size());
        assertSourceSpan(actorStmt.askHandlers().get(0), 37, 71);
    }
