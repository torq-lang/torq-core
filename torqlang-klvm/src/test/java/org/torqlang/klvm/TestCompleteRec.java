...
public class TestCompleteRec {
    ...
    public void testCreate() {
        ...
        assertInstanceOf(Map.class, nv);
        map = (Map<?, ?>) nv;
        assertEquals(Map.of(Rec.$LABEL, testLabel.value, Rec.$FIELDS, Map.of()), map);
        ...
        assertInstanceOf(Map.class, nv);
        map = (Map<?, ?>) nv;
        assertEquals(Map.of(Rec.$LABEL, testLabel.value, Rec.$FIELDS, Map.of(zero.value, a.value)), map);
        ...
        nv = r.toNativeValue();
        assertInstanceOf(Map.class, nv);
        Map<?, ?> innerNativeValue = Map.of(Rec.$LABEL, testLabel.value, Rec.$FIELDS, Map.of(zero.value, a.value));
        Map<?, ?> outerNativeValue = Map.of(Rec.$LABEL, anotherTestLabel.value, Rec.$FIELDS, Map.of(zero.value, b.value, one.value, innerNativeValue));
