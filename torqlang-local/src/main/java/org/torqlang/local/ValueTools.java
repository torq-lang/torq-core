...
public final class ValueTools {
    ...
    private static final String EPOCH_NAME = "epoch";

    private static boolean isLocalDateType(Type type) {
        // LocalDate type is an Obj intersection type
        if (!(type instanceof IntersectionType intersectionType)) {
            return false;
        }
        RecTypeExpr recTypeExpr;
        if (intersectionType.arg1 instanceof ObjType && intersectionType.arg2 instanceof RecTypeExpr) {
            recTypeExpr = (RecTypeExpr) intersectionType.arg2;
        } else if (intersectionType.arg2 instanceof ObjType && intersectionType.arg1 instanceof RecTypeExpr) {
            recTypeExpr = (RecTypeExpr) intersectionType.arg1;
        } else {
            return false;
        }
        // If the given type has a LocalDate constructor, then we have a LocalDate.
        // The signature for a LocalDate constructor is: func LocalDate(epoch::Int64) -> LocalDate.
        for (FieldType fieldType : recTypeExpr.fields) {
            if (fieldType.feature.value().equals(LocalDateMod.LOCAL_DATE_STR)) {
                if (fieldType.value instanceof FuncType funcType) {
                    if (!funcType.typeParams.isEmpty()) {
                        return false;
                    }
                    if (funcType.params.size() != 1) {
                        return false;
                    }
                    if (!(funcType.params.get(0) instanceof IdentAsPat param)) {
                        return false;
                    }
                    if (!param.ident.name.equals(EPOCH_NAME)) {
                        return false;
                    }
                    if (!(param.type instanceof Int64Type)) {
                        return false;
                    }
                    if (!(funcType.returnType instanceof IdentAsType returnType)) {
                        return false;
                    }
                    return returnType.ident().name.equals(LocalDateMod.LOCAL_DATE_STR.value);
                }
            }
        }
        return false;
    ...
    private static CompleteTuple toKernelTuple(Object label, List<?> values, Type type) {
        ...
        if (type != null) {
            if (type instanceof TypeCtor typeApply) {
