    ...
    public void test() throws Exception {
        String source = """
            actor SumArrayList() in
                import torq.lang.{Cell, ValueIter}
                import torq.util.{ArrayList as JavaArrayList}
                                ...
                                $create_proc(proc () in // free vars: $m
                                    local $v2 in
                                        local $v3 in
                                            ...
                                            $create_rec('error'#{'name': 'org.torqlang.lang.AskNotHandledError', 'message': 'Actor could not match request message with an \\'ask\\' handler.', 'details': $v3}, $v2)
                                        end
                                        throw $v2
                                    end
                                end, $else)
                        ...
                        $create_proc(proc ($m) in
                            local $v9 in
                                local $v10 in
                                    ...
                                    $create_rec('error'#{'name': 'org.torqlang.lang.TellNotHandledError', 'message': 'Actor could not match notify message with a \\'tell\\' handler.', 'details': $v10}, $v9)
                                end
                                throw $v9
                            end
                        end, $v8)
